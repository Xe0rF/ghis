...
- `build.rs` 将 Git commit、工作区状态、tag、构建时间、目标平台和构建模式写入版本信息；`src/` 是 Rust 核心代码：`main.rs` 提供 CLI 入口，`app.rs` 负责身份解析与仓库绑定，`config.rs` 管理配置和 XDG 路径，`agent.rs` 与 `src/agent/` 提供 Claude/Codex agent 上下文注入、启动器和设置管理，`git.rs`、`credential.rs`、`github.rs`、`signing.rs` 分别封装 Git、凭据、gh CLI 和 SSH 签名逻辑。
